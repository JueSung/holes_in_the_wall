...
var last_tick_on_floor: bool = true # on_floor from last tick - used for coyote time and landing animation
...
func _physics_process(delta: float) -> void:
	...
	if !is_on_floor():
		...
		if !last_tick_on_floor && abs(prev_velocity.y) > LAND_SPEED_THRESHOLD: # just landed
			set_animation("land", dir)
...
func set_animation(animation_name, dir):
	...
	$Hurtbox/CollisionShape2D.rotation = 0
	...
	match animation_name:
		...
		"dash":
			...
			$Hurtbox/CollisionShape2D.rotation = animatedSprite.rotation
			$CollisionShape2D.disabled = false
			$crouch_hitbox.disabled = true
			$Hurtbox/CollisionShape2D.disabled = false
			$Hurtbox/crouch_hitbox.disabled = true
...
# only really lasts if physics_process false cuz then just sets animation which does hitbox stuff
